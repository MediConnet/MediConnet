...
export const ProfileSection = ({ data, onUpdate }: ProfileSectionProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    schedule: data?.laboratory?.schedule || '',
  ...
  useEffect(() => {
    if (user?.id) {
      const savedImage = localStorage.getItem(`laboratory-profile-image-${user.id}`);
      if (savedImage) {
  ...
  const handleSave = async () => {
    ...
    localStorage.setItem(`laboratory-profile-${user.id}`, JSON.stringify(updatedData));
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        if (user?.id) {
          localStorage.setItem(`laboratory-profile-image-${user.id}`, base64String);
        }
      };
        ...
        {!isEditing ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <div>
                <label className="text-sm text-gray-600">Nombre del laboratorio</label>
                <p className="text-gray-800 font-medium mt-1">{laboratory.name}</p>
              </div>
              <div>
                <label className="text-sm text-gray-600">Email</label>
                <p className="text-gray-800 font-medium mt-1">{laboratory.email}</p>
              </div>
              <div>
                <label className="text-sm text-gray-600">WhatsApp</label>
                <p className="text-gray-800 font-medium mt-1">{laboratory.whatsapp}</p>
              </div>
              <div>
                <label className="text-sm text-gray-600">Dirección</label>
                <p className="text-gray-800 font-medium mt-1">{laboratory.address}</p>
              </div>
              <div>
                <label className="text-sm text-gray-600">Horario</label>
                <p className="text-gray-800 font-medium mt-1">{laboratory.schedule}</p>
              </div>
            </div>

            <div className="mt-6">
              <label className="text-sm text-gray-600">Descripción</label>
              <p className="text-gray-800 mt-2 leading-relaxed">{laboratory.description}</p>
            </div>
            ...
            {laboratory.workSchedule && laboratory.workSchedule.length > 0 && (
              <div className="mt-6">
                ...
                <div className="space-y-2">
                  {laboratory.workSchedule.map((schedule) => (
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <div>
                <label className="text-sm text-gray-600 mb-1 block">Nombre del laboratorio</label>
                <input
                  type="text"
                  value={formData.name}
        ...
        <div className="space-y-4">
          ...
          <div>
            <h4 className="text-lg font-bold text-gray-800">
              {isEditing ? formData.name : laboratory.name}
            </h4>
            <p className="text-sm text-gray-500 mt-1">Laboratorio Clínico</p>
          </div>
          ...
          <div className="flex items-center gap-2 text-gray-600">
            ...
            <span className="text-sm">
              {isEditing ? formData.address : laboratory.address}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 text-gray-600">
            ...
            <span className="text-sm">
              {isEditing ? formData.whatsapp : laboratory.whatsapp}
            </span>
          </div>
        </div>
      </div>
