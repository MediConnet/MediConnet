...
import type { ClinicSchedule, DaySchedule } from '../../clinic/types/clinic.entity';
...
} from '../../association/api/clinic-schedule.utils';
...
export const ClinicAssociatedScheduleSection = () => {
  ...
  const { clinicInfo, loading: loadingClinic } = useClinicAssociatedDoctor({ fetchProfile: false });
  ...
  const loadSchedules = useCallback(async () => {
    ...
    try {
      ...
      if (apiStatus === 404) {
        setError(
          clinicInfo
            ? 'El servicio de horarios para clínicas asociadas no está disponible. Contacta al administrador.'
            : 'No tienes una clínica asociada. Para gestionar horarios, primero debes estar asociado a una clínica.',
        );
      } else {
        setError('No fue posible cargar los horarios. Verifica tu conexión e intenta de nuevo.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError('No fue posible guardar tu disponibilidad. Verifica tu conexión e intenta de nuevo.');
    } finally {
      setSaving(false);
    }
  };

  if (loading || loadingClinic) {
