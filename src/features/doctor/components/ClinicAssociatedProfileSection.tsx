...
import type { ClinicAssociatedDoctorProfile } from "../../association/types/ClinicAssociatedDoctor.entity";
import { useClinicAssociatedDoctor } from "../../association/hooks/useClinicAssociatedDoctor";
import { updateClinicAssociatedProfileAPI } from "../../association/api/clinic-associated.api";
...
export const ClinicAssociatedProfileSection = ({
  ...
  const clinicAddress = _clinicInfo?.address || profile?.clinicInfo?.address || "Dirección no disponible";
  ...
  return (
    <Box>
      ...
      {/* Banner informativo de la clínica — siempre visible */}
      <Alert
        icon={<LocalHospital />}
        severity="info"
      ...
      {!loading && !profile && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          Los datos del perfil profesional no están disponibles temporalmente. La asociación con la clínica sigue activa.
        </Alert>
      )}
